...
public:
   ...
   LogicalResult matchAndRewrite(Operation* op, ArrayRef<Value> operands, ConversionPatternRewriter& rewriter) const override {
      ...
      // // auto elementPtr = rewriter.create<LLVM::GEPOp>(loc, targetPtrType, elemType, val, ValueRange{zero});
      // // elementPtr.dump();
      // // auto ref = rewriter.create<util::GenericMemrefCastOp>(op->getLoc(), i8PtrType, val);
      // // ref.dump();

      // // mlir::Value ptr;
      // // {
      // //    mlir::OpBuilder::InsertionGuard guard(rewriter);
      // //    auto* block = &op->getParentOfType<mlir::func::FuncOp>().getFunctionBody().front();
      // //    rewriter.setInsertionPointToStart(block);
      // //    auto elemType = typeConverter->convertType(rewriter.getI32Type());
      // //    auto elemPtrType = mlir::LLVM::LLVMPointerType::get(getContext());
      // //    auto size = rewriter.create<mlir::LLVM::ConstantOp>(loc, rewriter.getI64Type(), rewriter.getI64IntegerAttr(4));
      // //    ptr = rewriter.create<LLVM::AllocaOp>(loc, elemPtrType, elemType, size, 0);
      // // }

      // // rewriter.create<mlir::LLVM::StoreOp>(loc, val, ptr);
      // // ptr.dump();
      ...
      // //    rewriter.setInsertionPointToStart(block);
      // //    auto elemType = typeConverter->convertType(rewriter.getI32Type());
      // //    auto size = rewriter.create<arith::ConstantIndexOp>(loc, 4);
      // //    ptr = rewriter.create<util::AllocaOp>(loc, util::RefType::get(elemType), size);

      // //    // auto loweredType = mlir::cast<util::RefType>(elemType);
      // //    // mlir::Value typeSize = rewriter.create<util::SizeOfOp>(loc, rewriter.getIndexType(), loweredType.getElementType());
      // //    // ptr = rt::SimpleState::create(rewriter, loc)(mlir::ValueRange{typeSize})[0];
      // //    // ptr = rewriter.create<util::GenericMemrefCastOp>(loc, loweredType, ptr);
      // // }
      // // // ptr.dump();
      // // rewriter.create<util::StoreOp>(loc, val, ptr, mlir::Value());

      // auto targetPointerType = util::RefType::get(getContext(), rewriter.getI8Type());

      // // TODO
      // // Value strLen = rewriter.create<util::LoadOp>(loc, rewriter.getI32Type(), ptr, mlir::Value());
      // // printf("<<< strLen\n");
      // // strLen.dump();
      // // strLen.getType().dump();
      // // auto right = rewriter.create<arith::ConstantIntOp>(loc, 16, 32);
      // // right.getType().dump();
      // // Value cmpOp = rewriter.create<arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::ule, strLen, right);
      ...
      // // auto strPtr = rewriter.create<mlir::scf::IfOp>(
      // //                   loc, cmpOp, [&](mlir::OpBuilder& builder, mlir::Location loc) {

      // mlir::Type i128Ty = rewriter.getIntegerType(128);

      // Value v = rewriter.create<util::LoadOp>(loc, rewriter.getI64Type(), ptr, one);
      // printf("<<< IntToPtrOp\n");
      // v.dump();
      // v.getType().dump();
      // auto p = rewriter.create<util::IntToPtrOp>(loc, targetPointerType, v);
      // p.getType().dump();
